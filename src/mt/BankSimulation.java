	...
	public static void main(String args[]) {
		...
		for (t = 1; t <= numOfMinutes; t++) {
			...
			for (i = 0; i < clerksSize && !simulQ.isEmpty();)
				if (clerks[i] < 60) {
					x = (int) simulQ.dequeue(); // assign more than one customer
			...
			if (!simulQ.isEmpty()) {
				thereIsLine++;
				System.out.printf(" wait = ", ((long) (currWait / 60.0)) / 10);
				if (maxWait < currWait)
					maxWait = currWait;
			} else
				System.out.print(" wait = 0;");
		}
		System.out.println("\nFor " + clerksSize + " clerks, there was a line " + thereIsLine / numOfMinutes * 100.0
